...
export default function AdminDashboardScreen() {
  ...
  const styles = useMemo(() => StyleSheet.create({
    ...
    userName: { fontSize: 20, fontWeight: '700', color: theme.colors.text, marginTop: 2 },
    ...
    quickActionText: {
      fontSize: 12, fontWeight: '600', color: theme.colors.text, textAlign: 'center',
    },
    metricsScroll: { paddingLeft: 16, paddingRight: 8, gap: 12, marginBottom: 4 },
    metricCard: {
      backgroundColor: theme.colors.surface, borderRadius: 18, padding: 14,
    ...
    metricValue: { fontSize: 17, fontWeight: '700', color: theme.colors.text },
    ...
    cardTitle: { fontSize: 15, fontWeight: '700', color: theme.colors.text, marginBottom: 16 },
    ...
    counterNumber: { fontSize: 14, fontWeight: '700', color: theme.colors.text },
    counterLabel: { fontSize: 11, color: theme.colors.textSecondary, fontWeight: '600' },
    bigScore: { fontSize: 32, fontWeight: '700', color: theme.colors.text },
    scoreScale: { fontSize: 16, color: theme.colors.textSecondary },
    tableRow: {
      flexDirection: 'row', alignItems: 'center',
      paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: theme.colors.surfaceSecondary,
    },
    ...
    avatarInitial: { fontSize: 12, fontWeight: '700', color: theme.colors.textTertiary },
    guestName: { fontSize: 13, fontWeight: '600', color: theme.colors.text },
    propertySub: { fontSize: 11, color: theme.colors.textSecondary },
    dateText: { flex: 1, fontSize: 11, color: theme.colors.textSecondary },
      ...
      <View style={styles.metricHeader}>
        ...
        {trend && (
          <View style={styles.trendBadge}>
            <Feather name="trending-up" size={11} color={theme.colors.success} />
          </View>
        )}
      </View>
  ...
  const BookingRow = ({ name, property, date, status }: any) => (
    <View style={styles.tableRow}>
      ...
      <View style={[styles.statusBadge, {
        backgroundColor: status === 'Confirmed' ? theme.colors.successBg : theme.colors.warningBg,
      }]}>
    ...
    <SafeAreaView style={styles.container} edges={[]}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerActions}>
          <TouchableOpacity style={styles.notifButton} onPress={() => navigation.navigate('GuestMessages')}>
            <Feather name="bell" size={20} color={theme.colors.textTertiary} />
          </TouchableOpacity>
          <TouchableOpacity
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.section}>
          ...
          >
            <MetricCard title="Total Revenue" value="₱124,500" icon="currency-php" color={theme.colors.primary} trend />
            <MetricCard title="Active Bookings" value="12" icon="calendar-check" color="#8B5CF6" />
            <MetricCard title="Avg Rating" value="4.8" icon="star" color="#F59E0B" />
        ...
        <View style={[styles.card, { marginHorizontal: 16, marginBottom: 16 }]}>
          ...
          <View style={styles.countersRow}>
            <StatusCounter label="Check-ins" count="5" color={theme.colors.success} icon="login-variant" />
            <StatusCounter label="Check-outs" count="3" color={theme.colors.error} icon="logout-variant" />
            ...
            <View style={{ flexDirection: 'row', gap: 4, marginVertical: 8 }}>
              {[1, 2, 3, 4, 5].map(i => (
                <MaterialCommunityIcons
                  ...
                  name="star"
                  size={18}
                  color={i <= 4 ? "#F59E0B" : theme.colors.surfaceSecondary}
                />
              ))}
            </View>
            <Text style={{ fontSize: 12, color: theme.colors.textSecondary }}>Total Reviews: 128</Text>
          </View>
        </View>
        ...
        <View style={[styles.card, { marginHorizontal: 16, marginBottom: 32 }]}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 16 }}>
            <View style={[styles.statPill, { backgroundColor: theme.colors.primaryLight }]}>
              <MaterialCommunityIcons name="office-building" size={14} color={theme.colors.primary} />
              <Text style={{ fontSize: 11, fontWeight: '700', color: theme.colors.primaryDark }}>4 Listed</Text>
            </View>
          </View>
          <Text style={{ fontSize: 12, color: theme.colors.textSecondary, marginBottom: 12, fontWeight: '600' }}>
            Weekly Occupancy
          </Text>
          <View style={styles.barChart}>
            {[40, 70, 55, 90, 85, 60, 75].map((h, i) => (
              <View
                ...
                style={[styles.bar, {
                  height: h * 0.55,
                  backgroundColor: i === 3 ? theme.colors.primary : theme.colors.primary + '40',
                }]}
              />
            ))}
          </View>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 6 }}>
            {['M', 'T', 'W', 'T', 'F', 'S', 'S'].map((d, i) => (
              <Text key={i} style={{ fontSize: 10, color: theme.colors.textTertiary, textAlign: 'center', flex: 1 }}>{d}</Text>
            ))}
          </View>
        </View>
      </ScrollView>
